...
source_file = r"C:\Users\DHARMENDAR\.gemini\antigravity\brain\f1ddc42a-873f-4f9d-8f44-2303b4839e6d\.system_generated\steps\8\content.md"
    ...
    for match in li_pattern.finditer(html_block):
        ...
        if title and href and img_src:
            ...
            if not img_src.startswith("http"):
                full_img_src = "https://www.5movierulz.pictures" + ("/" if not img_src.startswith("/") else "") + img_src
            ...
            # Keep the href to 5movierulz.pictures as requested
            # Normalize to https://www.5movierulz.pictures/ path
            normalized_href = href
            if not href.startswith("http"):
                normalized_href = "https://www.5movierulz.pictures" + ("/" if not href.startswith("/") else "") + href
...
# Also update any other movie links (.vote, .support, etc.) to .pictures
target_content = re.sub(r'https?://www\.5movierulz\.(vote|support|house|online)/', 'https://www.5movierulz.pictures/', target_content)

with open(target_html, 'w', encoding='utf-8') as f:
    f.write(target_content)

print("Updated index.html successfully with new movies and sidebar links pointing to 5movierulz.pictures.")
